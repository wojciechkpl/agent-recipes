...
- **`asana-sync`** (subrecipe) — best-effort Asana task sync; optional side-channel in
  `wf-feature`, `wf-bugfix`, `wf-spec`. Preflights availability; silent no-op if Asana
  isn't configured/reachable (resolve project via caller / `ASANA_PROJECT_GID` / `.claude/asana.json`).
